...
const COLOR_HEX: Record<CharacterColor, string> = {
  blue: "#00e5ff", red: "#ff4d6d", green: "#39ff14", purple: "#bf5af2", gold: "#ffd60a",
};

const SKIN_LABELS: Record<CharacterSkin, string> = {
  adventurer: "AVENTUREIRO", nomad: "NÔMADE", explorer: "EXPLORADOR", wanderer: "ERRANTE",
};
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      (dbPins ?? []).forEach((p) => {
        ...
        pinsByCountry[p.country_code].push({ id: p.id, type: p.type as PinType, name: p.name, lat: p.lat, lng: p.lng, countryCode: p.country_code, note: p.note ?? undefined });
      });
      setCountries(
        (dbCountries ?? []).map((c) => ({
          code: c.code, name: c.name, visited: c.visited, visitedAt: c.visited_at ?? undefined,
          photoUrl: c.photo_url ?? undefined,
          flagEmoji: c.flag_emoji, continent: c.continent, pins: pinsByCountry[c.code] ?? [],
        }))
      );
  ...
  async function saveCharacter() {
    setSaving(true);
    const supabase = createClient();
    await supabase.from("users").update({ character: character as unknown as Json }).eq("id", user.id);
    setSaving(false);
    setSaved(true);
        ...
        <Panel title="PERSONAGEM" accent="#ffd60a">
          <div className="flex gap-5 items-start">
            ...
            <div className="flex-1 flex flex-col gap-[10px]">
              ...
              <div className="text-[8px]" style={{ color: COLOR_HEX[color] }}>{SKIN_LABELS[skin]}</div>
        ...
        <Panel title="PERSONALIZAR" accent="#bf5af2">
          <div className="mb-[18px]">
            ...
            <div className="flex flex-wrap gap-2">
              {SKINS.map((s) => {
                const active = skin === s;
                return (
                  <button key={s} onClick={() => setSkin(s)} className="font-pixel text-[7px] px-[10px] py-2 cursor-pointer"
                    style={{ border: `2px solid ${active ? "#ffd60a" : "#1e6ea8"}`, background: active ? "#ffd60a22" : "#0277bd", color: active ? "#ffd60a" : "rgba(255,255,255,0.5)", boxShadow: active ? "none" : "2px 2px 0 #01579b", transform: active ? "translate(2px,2px)" : "none" }}>
                    {active ? "► " : ""}{SKIN_LABELS[s]}
                  </button>
                );
              })}
            </div>
          ...
          <div className="mb-[18px]">
            ...
            <div className="flex gap-[10px] items-center">
              {COLORS.map((c) => (
                <button key={c} onClick={() => setColor(c)} title={c} className="w-8 h-8 cursor-pointer"
                  style={{ background: COLOR_HEX[c], border: `3px solid ${color === c ? "#ffffff" : "#01579b"}`, boxShadow: color === c ? `0 0 0 2px #ffffff44, 3px 3px 0 #01579b` : "2px 2px 0 #014080", transform: color === c ? "none" : "translate(0,0)" }}
                />
              ))}
            </div>
          </div>
          <div className="mb-[18px]">
            ...
            <div className="flex gap-[10px]">
              {[
                ...
                <button key={acc.key} onClick={() => acc.set(!acc.value)} className="font-pixel text-[7px] px-[14px] py-[10px] cursor-pointer"
                  style={{ border: `2px solid ${acc.value ? "#bf5af2" : "#1e6ea8"}`, background: acc.value ? "#3d0a6b" : "#0277bd", color: acc.value ? "#bf5af2" : "rgba(255,255,255,0.5)", boxShadow: acc.value ? "none" : "2px 2px 0 #01579b", transform: acc.value ? "translate(2px,2px)" : "none" }}>
                  {acc.label}
                </button>
              ))}
            </div>
          ...
          <button
            ...
            style={{ borderColor: saved ? "#39ff14" : "#bf5af2", background: saved ? "#39ff1411" : "#bf5af211", color: saved ? "#39ff14" : "#bf5af2" }}
          >
      ...
      <Panel title="ESTATÍSTICAS" accent="#00e5ff">
        <div className="grid grid-cols-2 gap-3">
          {[
            ...
            <div key={s.label} className="bg-[#01579b] shadow-pixel-md px-4 py-[14px] flex items-center gap-[14px]" style={{ border: `2px solid ${s.color}33` }}>
              <span className="text-[28px] leading-none">{s.icon}</span>
              <div>
                <div className="text-[22px] mb-1" style={{ color: s.color }}>{s.value}</div>
        ...
        {countries.length === 0 ? (
          ...
          <div>
            {countries.map((country, i) => (
              <div key={country.code} className="flex items-center gap-4 px-5 py-[14px]"
                style={{ borderBottom: i < countries.length - 1 ? "1px solid #29b6f620" : "none", background: i % 2 === 0 ? "#0277bd44" : "transparent" }}>
                <CountryFlag code={country.code} size={36} />
                <div className="flex-1">
                  <div className="text-[10px] text-[#ffffff] mb-1">{country.name}</div>
                  <div className="text-[7px] text-white">{country.pins.length} pins · {country.continent}</div>
                </div>
                <div className="flex gap-1">
                  {country.pins.slice(0, 5).map((p) => <PixelPin key={p.id} type={p.type} size={20} />)}
                </div>
              </div>
            ))}
          </div>
