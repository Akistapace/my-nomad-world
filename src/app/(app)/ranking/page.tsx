...
function LeaderboardTable({ rows, showPodium }: { rows: RankPlayer[]; showPodium: boolean }) {
  ...
  return (
    <>
      {showPodium && top3.length >= 3 && (
        <Panel noPad>
          <div className="bg-[linear-gradient(180deg,#ffd60a11_0%,transparent_60%)] px-4 pt-6 pb-0">
            <div className="text-[9px] text-[#ffd60a] text-center mb-5 tracking-[3px]">
              ★ TOP 3 ★
            </div>
              ...
              {[top3[1], top3[0], top3[2]].map((player, i) => {
                ...
                return (
                  <div
                    ...
                    style={{ flex: rank === 1 ? "0 0 140px" : "0 0 110px" }}
                  >
                    <span className="text-2xl">{MEDAL_ICON[mIdx]}</span>
                    <div
                      className="bg-[#01579b] p-2"
                      style={{ border: `2px solid ${MEDAL_COLOR[mIdx]}` }}
                    >
                      <PixelCharacter character={player.character} size={charSizes[i]} />
                    </div>
                    <div
                      className="text-center leading-[1.6] max-w-[120px]"
                      style={{ fontSize: rank === 1 ? 8 : 7, color: MEDAL_COLOR[mIdx] }}
                    >
                      @{player.username}
                      {player.isMe && " ◄"}
                    </div>
                    ...
                    <div
                      ...
                      style={{
                        height: heights[i],
                        background: `${MEDAL_COLOR[mIdx]}18`,
                        border: `2px solid ${MEDAL_COLOR[mIdx]}`,
                        borderBottom: "none",
                      }}
                    >
                      <span className="text-[22px]" style={{ color: MEDAL_COLOR[mIdx] }}>
                        #{rank}
                      </span>
                    </div>
                  </div>
      ...
      <Panel noPad>
        <div
          className="grid gap-2 px-5 py-[10px] bg-[#01579b] border-b-2 border-b-[#29b6f644] text-[7px] text-white tracking-[1px]"
          style={{ gridTemplateColumns: "52px 44px 1fr 80px 80px" }}
        >
          <div>RANK</div>
          <div></div>
          <div>JOGADOR</div>
          <div className="text-center">PAÍSES</div>
          <div className="text-right">XP</div>
        </div>
        {rows.length === 0 && (
          <div className="px-5 py-8 text-[8px] text-white/40 text-center">
        ...
        {rows.map((player, i) => (
          <div
            key={player.id}
            className="grid items-center gap-2 px-5 py-[10px]"
            style={{
          ...
          >
            <div
              className="text-[9px]"
              style={{ color: player.isMe ? "#00e5ff" : "rgba(255,255,255,0.5)" }}
            >
              #{player.rank}
            </div>
            <div>
              <PixelCharacter character={player.character} size={36} />
            </div>
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                {player.isMe && (
                  <span className="pixel-badge text-[#00e5ff] border-[#00e5ff] bg-[#00e5ff11]">
                    VOCÊ
                  </span>
                )}
              </div>
...
export default function RankingPage() {
  ...
  async function loadFriends() {
    ...
    const meEntry: RankPlayer = {
      ...
      xp: user.xp,
      rank: user.rank,
      countriesCount: countByUser[user.id] ?? 0,
      isMe: true,
    };
    ...
    const sorted = [meEntry, ...friendEntries]
      .sort((a, b) => b.xp - a.xp)
      .map((p, i) => ({ ...p, rank: i + 1 }));
