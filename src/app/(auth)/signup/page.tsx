...
import { COUNTRIES_CATALOG, countryFlagEmoji, type CountryOption } from "@/lib/countries-catalog";
...
export default function SignupPage() {
  ...
  const filtered = COUNTRIES_CATALOG.filter((c) =>
    !search ||
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    if (!selected) { setError("Selecione seu país de origem"); return; }
    setLoading(true);
    setError(null);
        ...
        <div className="pixel-panel overflow-hidden">
          ...
          <div className="px-6 py-7">
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              <div>
                ...
                <input name="username" type="text" required minLength={3} maxLength={20}
                  placeholder="NomadeViajante" className="w-full pixel-input" />
              </div>
              ...
              <div>
                ...
                <div className="pixel-input-wrap">
                  <input name="password" type={showPass ? "text" : "password"} required minLength={6}
                    placeholder="min. 6 caracteres" className="px-3.5 py-3 pr-10" />
              ...
              <div>
                <label className="mb-2 block text-[7px] tracking-[1px] text-white">🏠 PAÍS DE ORIGEM</label>
                <div ref={wrapRef} className="relative">
                  {/* Search — always visible */}
                  <div className="pixel-input-wrap">
                    <span className="pl-3 text-white/30 text-sm leading-none select-none shrink-0">🔍</span>
                    <input
                      ref={searchRef}
                      type="text"
                      value={search}
                      onChange={(e) => { setSearch(e.target.value); setOpen(e.target.value.length > 0); }}
                      onFocus={(e) => { if (e.target.value.length > 0) setOpen(true); }}
                  ...
                  {/* Dropdown */}
                  {open && (
                    <div className="absolute z-50 w-full border-2 border-sky-400 border-t-0 bg-[#01579b] shadow-[3px_3px_0_#014080]"
                      ...
                      {filtered.map((c, i) => (
                        <button
                          ...
                          style={{ borderBottom: i < filtered.length - 1 ? "1px solid rgba(41,182,246,0.15)" : "none" }}
                          onMouseEnter={(e) => (e.currentTarget.style.background = "#0277bd")}
                          onMouseLeave={(e) => (e.currentTarget.style.background = "transparent")}
                        >
                          <FlagImg alpha2={c.alpha2} size={20} />
                          <span className="flex-1 text-[9px] text-slate-100 truncate">{c.name}</span>
                          <span className="text-[6px] text-white/40 shrink-0">{c.continent}</span>
                        </button>
                      ))}
                      {filtered.length === 0 && (
                        <div className="px-4 py-4 text-[8px] text-white/40 text-center">Nenhum resultado</div>
                      )}
                    </div>
                  )}
                </div>
            ...
            <div className="mt-6 text-center text-[7px] text-white">
              JÁ TEM CONTA?{" "}
              <Link href="/login" className="text-cyan-300 no-underline">ENTRAR</Link>
            </div>
          </div>
        </div>
      </div>
