...
const NAV_ITEMS = [
  ...
  { href: "/challenges", label: "Missões", icon: "⚡" },
  ...
  return (
    <aside className="app-sidebar w-60 bg-[#0277bd] border-r-[3px] border-r-[#29b6f6] shadow-[3px_0_0_#01579b] flex-col sticky top-0 h-[100dvh] overflow-y-auto shrink-0">
      ...
      <div className="pt-5 px-4 pb-4 border-b-2 border-b-[#29b6f644] bg-[linear-gradient(180deg,#01579b_0%,transparent_100%)]">
        <div className="text-[9px] text-[#ffffff] tracking-[2px] mb-1">
          ◆ MY NOMAD
        </div>
        ...
        <div className="grid grid-cols-2 gap-1.5">
          {[
            ...
            <div key={s.label} className="bg-[#01579b] px-2 py-1.5 border border-white">
              <div className="text-sm" style={{ color: s.color }}>{s.value}</div>
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <Link
              ...
              onMouseEnter={(e) => { if (!active) (e.currentTarget as HTMLElement).style.background = "#01579b44"; }}
              onMouseLeave={(e) => { if (!active) (e.currentTarget as HTMLElement).style.background = "transparent"; }}
            >
              <span className="text-lg leading-none">{item.icon}</span>
              <span className={`font-pixel text-[9px] tracking-[0.5px] ${active ? "text-[#00e5ff]" : "text-white"}`}>
      ...
      <div className="px-4 py-3 border-t-2 border-t-[#29b6f644] flex flex-col gap-2">
        <form action={logout}>
          <button
            ...
            onMouseEnter={(e) => { e.currentTarget.style.background = "#ff4d6d11"; e.currentTarget.style.borderColor = "#ff4d6d"; }}
            onMouseLeave={(e) => { e.currentTarget.style.background = "transparent"; e.currentTarget.style.borderColor = "#ff4d6d44"; }}
          >
            ⏻ SAIR
          </button>
        </form>
